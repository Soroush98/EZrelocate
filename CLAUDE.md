...
Apify actor scraping rental listings — Canada (Kijiji, RentFaster.ca), the UK
(OpenRent), and the US (Zumper) — into one normalized, deduplicated dataset
with offline nearest-amenity enrichment. The pipeline is country-aware (the
registry in `src/sources/__init__.py` is the seam for adding sources/countries;
Australia is blocked by anti-bot walls, see PROJECT_NOTES). Python, no
database — listings go straight to the Apify dataset. Deploy is manual via
`apify push`.
